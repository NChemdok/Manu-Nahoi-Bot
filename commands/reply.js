...
const reply = (message, playerQueue) => {
  ...
  if (messageText === "start") {
    ...
    if (!lookup(user.username)) {
      message.channel.send(
        "Game start hoishe, Enika Nahoi koile (*saap start) type kuribi"
      );
      ...
      playerQueue.forEach((player) => {
        ...
        if (player.value >= 100) {
          const winnerDeclaration = new Discord.MessageEmbed()
            ...
            .setDescription(player.key + " Won !!! Game has been Reset");
...
// message.channel.send("Yate Eku wi Sawole nai");
